...
AcfUxCollapse.FieldTypes.Repeater = (function ($) {
	function Repeater() {
		// Initialize
		$(
			'div.inside.acf-fields.-top > .acf-field-repeater[data-type="repeater"]',
		).each(
		...
		$(document).on(
			"click",
			'.acf-field-repeater[data-type="repeater"] .acf-collapser-initialized .acf-row-handle.order',
			function (e) {
	...
	Repeater.prototype.init = function (field) {
		...
		var $cloneRow = $field.find(
			"> .acf-input > .acf-repeater > table > tbody > .acf-row.acf-clone",
		);
		$cloneRow.removeClass("acf-clone");

		$field.find("> .acf-input > .acf-repeater > table > tbody > .acf-row").each(
			function (index, element) {
				...
				$(element)
					.find(".acf-fields")
					.wrapInner(
						'<div class="acf-collapser-collapser-area acf-fields"></div>',
					);
				$(element)
					.find(".acf-row-handle.order")
					.append('<div class="acf-collapser-icon" data-text="Expand"></div>');
